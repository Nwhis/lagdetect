...
net.Receive("lagdetect_notify",function()
    ...
    if not islag then
        ...
        bs.Notify("[LagDetect] Physics timescale restored!",3,Color(50,255,0),true)
    ...
    bs.Notify("[LagDetect] Physics timescale set to "..tostring(math.Round(speed,2)),5,Color(255,200,0),true)
    ...
    bs.Notify("[LagDetect] "..n.." has "..tostring(num).." intersecting props ("..tostring(percent).."%)",8,Color(255,125,0),true)
    MsgC(p,m,c,n,
        msgcolor," [",s,"] has ",
        Color(255,60,40),num,
